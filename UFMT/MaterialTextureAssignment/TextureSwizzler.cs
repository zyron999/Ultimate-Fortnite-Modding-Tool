...
namespace UFMT.MaterialTextureAssignment
{
    ...
    {
        internal static bool SwizzleSpecularTextures(string texturesPath, string[] textures)
        {
            try
            {
                ...
                HashSet<string> texturePaths = textures.Select(tex => Path.Combine(texturesPath, $"{tex}.png")).Where
                (texPath => File.Exists(texPath) && !File.Exists(Path.Combine(swizzledTexturesPath, Path.GetFileName(texPath)))).ToHashSet();
            ...
            {
                Log.Error($"An error occured while trying to swizzle specular textures: {ex.Message}");
                return false;
            }
        }
    }
